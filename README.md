...
依托于这个星球上最强大的推理模型（Google Gemini 3.0 家族），本工具能以前所未有的速度和极低的成本，彻底替代传统的人工看周报流程。
...
2. **多模态结构化洞察 (`src/analyzer.py`)**：
   - **海量并发拆解**：动用 15 组并发线程，指派极速预览版视觉模型 (`gemini-3-flash-preview`) 对这 60 个单独的视频进行逐帧拆解。核心提取 5 大维度：*前3秒钩子设计、情绪导向、内容结构、核心爆点/爽点、文案特征*。
   - **降本增效黑科技**：全面采用 v1alpha 版本的新型 SDK 分配器，将视频处理锁死在极其廉价的低分辨率（`media_resolution_low`），大幅榨取 API 性能，单周分析视频的账单总额被压缩至 $0.30 级别！
3. **宏观战略大盘研判 (`src/analyzer.py`)**：剥离庞杂的视频原生数据后，将 60 份高度压缩的文本情报统一喂给新一代最强推理巨兽 (`gemini-3.1-pro-preview`)，由它以“买量战略总监”的视角，萃取全渠道投放规律、提炼竞品创新打法，并对下周的素材团队提出**落地方向建议**。
4. **终极视觉呈现 (`src/renderer.py`)**：基于定制的高级 HTML/CSS 模板构建，为这份冰冷的纯文字洞察披上美观、易读的网页高亮外衣。
...
python main.py
```
终端跑完进度条后，去根目录下的 `reports/` 文件夹里，双击打开最新生成的 `.html` 网页文件即可享用你的 AI 大作！
