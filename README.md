...
依托于高效且低成本的多模态推理模型（Google Gemini 2.5 Flash），本工具能以前所未有的速度，彻底替代传统的人工看周报流程。
...
2. **多模态结构化洞察 (`src/analyzer.py`)**：
   - **海量并发拆解**：动用 15 组并发线程，指派视觉模型 (`gemini-2.5-flash`) 对这 60 个单独的视频进行逐帧拆解。核心提取 5 大维度：*前3秒钩子设计、情绪导向、内容结构、核心爆点/爽点、文案特征*。
