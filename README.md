...
**在线预览：** [https://kevqwe.github.io/slg-ad-weekly-analyzer](https://kevqwe.github.io/slg-ad-weekly-analyzer)
...
1. **情报收集 (`src/fetcher.py`)**：从全球顶尖的数据分析平台 **Sensor Tower** 自动拉取过去 7 天内，美国区双端（iOS/Android）表现最顶级的 SLG 视频广告（涵盖 Applovin, Facebook, YouTube 各平台 Top 50）。精准锁定 14 款核心竞品，捕获其素材的**曝光份额 (Impression Share)** 并标注发行商公司信息。
...
3. **排名趋势追踪与持久化 (`main.py`)**：自动读取上一周的历史榜单存档数据并比对，追踪每个爆款素材的**周排名变化**（上升、下降、持平、新上榜）及**曝光份额环比变化**。所有原始数据以及分析中间结果通过 GitHub Action 自动归档至 `archive/` 目录以便做永久追溯，更有**本地智能缓存机制 (Local Cache Fallback)** 有效节省同一视频重复分析的 API 成本。
...
5. **终极视觉呈现 (`src/renderer.py`)**：基于定制的高级 HTML/CSS 模板构建，为这份冰冷的纯文字洞察披上美观、易读的网页高亮外衣。生成的单网页完全支持双击本地运行，更具备以下交互能力：
   - **本周新素材速览**：可折叠的快捷入口面板，一键跳转到对应频道的详细拆解
   - **游戏筛选 Chip**：按游戏名筛选各频道素材，快速定位目标竞品
   - **常驻左侧导航栏**：快速在竞品分析、各频道 Top 50 间跳转
   - **发行商标签**：每条素材标注所属公司，清晰标明竞品归属
   - **GA4 数据埋点**：内置 Google Analytics 追踪用户行为，量化工具使用情况
...
```bash
# 克隆或下载本仓库
git clone https://github.com/KEVqwe/slg-ad-weekly-analyzer.git
cd slg-ad-weekly-analyzer
